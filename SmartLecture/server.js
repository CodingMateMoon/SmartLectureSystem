...
function serverHandler(request, response) {
    try {
        ...
        try {
            ...
            if (filename && filename.search(/smartlecture/g) === -1 && stats.isDirectory()) {
                response.writeHead(200, {
                    'Content-Type': 'text/html'
                });
                response.write('<!DOCTYPE html><html><head><meta http-equiv="refresh" content="0;url=/smartlecture/"></head><body></body></html>');
                response.end();
                return;
            }
        } catch (e) {
        ...
        if (fs.statSync(filename).isDirectory()) {
            ...
            } else if (filename.indexOf(resolveURL('/smartlecture/')) !== -1) {
                filename = filename.replace(resolveURL('/smartlecture/'), '');
                filename += resolveURL('/smartlecture/index.html');
            } else {
                filename += resolveURL('/smartlecture/index.html');
            }
        }
        ...
        fs.readFile(filename, 'binary', function(err, file) {
            ...
            try {
                var demos = (fs.readdirSync('smartlecture') || []);
                ...
                if (demos.length) {
                    ...
                    demos.forEach(function(f) {
                        if (f && f !== 'index.html' && f.indexOf('.html') !== -1) {
                            otherDemos += '<li><a href="/smartlecture/' + f + '">' + f + '</a> (<a href="https://github.com/muaz-khan/RTCMultiConnection/tree/master/demos/' + f + '">Source</a>)</li>';
                        }
                    });
...
function runServer() {
    ...
    app = app.listen(port, process.env.IP || '0.0.0.0', function(error) {
        ...
        console.log('------------------------------');
        console.log('SmartLectureSystem');
    });
